...
public class FuncionariosDAO {
    ...
    public static List<FuncionariosVO> listarVendas(int tmpTipo) throws Exception {
        ...
        try {
            ...
            sqlVendas = "SELECT orders.CustomerID, orders.ShipName, orders.OrderID, orders.EmployeeID, sum(orderdetails.UnitPrice * orderdetails.quantity) as Total FROM orders "
                    + "INNER JOIN orderdetails ON orders.OrderID = orderdetails.OrderID where orders.EmployeeID = " + tmpTipo + " group by orders.orderid order by orders.orderid";
            ...
            while (rsVendas.next()) { //enquanto houver clientes no result
                ...
                tmpVendas.setPreco(rsVendas.getFloat("Total"));
